...
async def restart(client, message):
    ...
    # Command to get DC of a user
async def dc_user(client, message):
    try:
        username = message.text.split()[1] if len(message.text.split()) > 1 else None
        if username:
            user = await client.get_users(username)
            dc_id = user.dc_id  # Get the DC ID for the user
            await sendMessage(message, f"The DC of the user {username} is: {dc_id}")
        else:
            await sendMessage(message, "Please provide a valid username.")
    except PeerIdInvalid:
        await sendMessage(message, "Could not find the user. Please check the username.")
...
async def main():
    ...
    # Add new handlers for DC functionality
    bot.add_handler(MessageHandler(dc_user, filters=command("dc_user")))
    bot.add_handler(MessageHandler(dc_file, filters=command("dc_file")))
